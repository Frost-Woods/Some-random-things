...
function initColorSettings() {
  ...
  colorSettings.innerHTML = ''; // 清空原有内容，避免重复
  ...
  syntaxElements.forEach(element => {
    ...
    // 修复 HTML 结构，确保语言标签正常渲染
    settingDiv.innerHTML = `
      <div class="color-setting-row">
        <span class="color-setting-label">${element.name}</span>
        <div class="color-picker-container">
          <div class="color-preview" style="background-color: ${currentColor};" id="${element.id}Preview"></div>
...
function applyColorSettings() {
  ...
  syntaxElements.forEach(element => {
    ...
    // 处理带点的类名（如 title.function_），拼接为 .hljs-title.function_
    const hljsClass = element.id.includes('.') 
      ? element.id.split('.').map(part => `hljs-${part}`).join('.')
      : `hljs-${element.id}`;
    css += `[data-theme="${theme}"] .${hljsClass} { color: ${color} !important; }\n`;
  });
